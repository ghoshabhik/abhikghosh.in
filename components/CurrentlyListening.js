...
export default function CurrentlyListening() {
    ...
    const { data } = useSWR(`/api/get-spotify-favplaylist`, fetcher, {refreshInterval: 1000})
      ...
      (a, b) =>
        Number(new Date(b.popularity)) - Number(new Date(a.popularity))
                    ...
                    <div key={index} className="flex justify-between mb-2">
                        <p className="text-black dark:text-gray-100"><a href={track.song.url} rel="nofollow noreferrer"  target="_blank">{track.song.track}</a> - <span className="text-md text-gray-500 dark:text-gray-400">{track.song.artist}</span></p>
